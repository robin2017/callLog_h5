function handle() {
    ...
    const {eachDayHours,eachDays,eachHours} = window.globalObject;
    ...
    Object.keys(eachDayHours).forEach(item=>{
        var strs = item.split('*');
       //data.push([strs[0],strs[1],eachDayHours[item]])
        data.push([days.indexOf(strs[0]),hours.indexOf(strs[1]),eachDayHours[item]])
    })
    console.log('data:===',days,hours,data);
    console.log('test:',days.indexOf('7-3'))
